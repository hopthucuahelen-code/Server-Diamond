...
if not exist "jre\bin\java.exe" (
    ...
    powershell -NoProfile -ExecutionPolicy Bypass -Command ^
      "$ErrorActionPreference='Stop'; ^
       ...
       $zip='jre.zip'; ^
       Write-Host 'Dang tai Java 21 tu Adoptium...' -ForegroundColor Cyan; ^
       [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; ^
       Invoke-WebRequest -Uri $url -OutFile $zip; ^
       Write-Host 'Dang giai nen...' -ForegroundColor Cyan; ^
       Expand-Archive -Path $zip -DestinationPath 'temp_jre'; ^
       $extracted = Get-ChildItem -Directory -Path 'temp_jre' | Select-Object -First 1; ^
       Move-Item -Path $extracted.FullName -Destination 'jre'; ^
       Remove-Item -Path 'temp_jre' -Recurse -Force; ^
       Remove-Item -Path $zip -Force; ^
       Write-Host 'Tai va cai dat Java portable hoan tat!' -ForegroundColor Green"
    if errorlevel 1 (
...
start "" /b powershell -NoProfile -WindowStyle Hidden -Command ^
  "for($i=0;$i -lt 120;$i++){try{$c=New-Object Net.Sockets.TcpClient;$c.Connect('localhost',8080);$c.Close();Start-Process 'http://localhost:8080';break}catch{Start-Sleep -Milliseconds 1000}}"
